  ...
  def from_data(cls, data : dict) -> Self:
    ...
    for attribute in data:
      match attribute:
        ...
        case "avatar":
          user.__dict__[attribute] : str = CallableAvatar(f"https://cdn.discordapp.com/avatars/{data["id"]}/{data["avatar"]}.{"gif" if data["avatar"].startswith("a_") else "png"}") if data[attribute] else None
          user.__dict__[attribute].default_avatar : str = f"https://cdn.discordapp.com/embed/avatars/{(int(data["id"]) >> 22) % 6 if int(data["discriminator"]) == 0 else int(data["discriminator"]) % 5}.png"
          user.__dict__[attribute].avatar_decoration : str = f"https://cdn.discordapp.com/avatar-decoration-presets/{data["avatar_decoration_data"]["asset"]}.png" if data["avatar_decoration_data"] else ""
